...
- **`httpspec` has zero importers** — built, tested, documented, unused externally. The module-split question is closed (nobody needs it versioned apart); the real gap is **discovery/marketing**, which belongs to the README/docs-site workstream, not to go.mod surgery. The flat-root "second consumer asks for internal/ hygiene" trigger is now _partially armed_: consumer count already exceeds two, so any one of them asking fires it.
