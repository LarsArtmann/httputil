...
- **v1.2.0 (next minor):** Three behavior changes within unchanged signatures: CSRF `TrustedOrigins` parsing is fail-closed all-or-nothing and `Validate` rejects entries that are not usable `scheme://host` origins; CSRF `SameSite=None` without `Secure` falls back to `Secure=true` at construction (`AllowInsecureSameSiteNone` opts out); and the `http.ErrNoCookie`/`http.ErrNoLocation` classifications moved Transient → Rejection. See [migrating-to-v1.2.md](migrating-to-v1.2.md).
