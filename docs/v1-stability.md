...
- **v1.2.0 (next minor):** Two behavior changes within unchanged signatures: CSRF `TrustedOrigins` parsing is fail-closed all-or-nothing and `Validate` rejects entries that are not usable `scheme://host` origins, and the `http.ErrNoCookie`/`http.ErrNoLocation` classifications moved Transient → Rejection. See [migrating-to-v1.2.md](migrating-to-v1.2.md).
