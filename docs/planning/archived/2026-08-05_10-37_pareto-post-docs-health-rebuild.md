...
1. ~~**CHANGELOG freeze policy** (M9/F9.1): Freeze `[0.8.0]` now (immutable, corrections go to `[Unreleased]`), or allow continued post-release refinement?~~ done (decided: freeze policy documented in AGENTS.md)
2. ~~**Annotate scope** (M2): Annotate all 6 reports now, or only 07-45 (the one with a proven lie)? The other 5 are accurate snapshots from today.~~ done (executed: all annotated by later passes)
3. ~~**Execution mode**: Execute the full plan now ("GET SHIT DONE"), or stop here and let the user pick which tasks to run?~~ done (executed in full (11-26 report))
