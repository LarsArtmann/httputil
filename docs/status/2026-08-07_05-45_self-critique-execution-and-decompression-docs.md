...
**Starting commit:** `994d030` (prior session's status report annotation)
...
3. **`nix flake check` run** — All checks pass (treefmt, devShells, packages, apps, formatter). This was the 4th consecutive session that skipped it; the gap is closed. The check evaluates in ~15s and confirms flake correctness.

### Tests

4. **Decompression bomb-protection tests** (`decompression_test.go`, commit `54afaa7`) — Three new tests:
...
5. **ETag edge-case tests** (`etag_test.go`, commit `3ee2781`) — Two new tests:
...
6. **`ExampleDecompression`** (`example_test.go`, commit `d0f9e7f`) — Testable example with `// Output:` directive. Compresses "hello decompression" with gzip, wraps a handler with `Decompression`, verifies the decompressed body reaches the handler. Consistent with all other middleware examples.

### Documentation

7. **Decompression added to README.md** (commit `d0f9e7f`) — Six additions:
...
1. **Regenerate D2 SVG** from updated `.d2` source. `docs/architecture-understanding/`. Effort: 5min (if `d2` CLI available).
2. **Add honest-silence comment to 6 remaining error-swallow sites** — `csrf.go:246`, `ratelimit.go:190`, `ratelimit_keyed.go:204`, `recovery.go:27`, `compression.go:215`, `decompression.go:136`. Effort: 15min.
3. **Update FEATURES.md timestamp** — line 5 description to reflect this session's work. Effort: 2min.
4. **Update AGENTS.md Non-Obvious Behaviors** — document the honest-silence pattern for post-header-commit writes. Effort: 10min.
5. **Clear LSP cache / restart LSP** — eliminate phantom `etag_httpspec_test.go` warnings. Effort: 2min.
6. **Run `art-dupl`** — verify no new duplication after `writeBufferedBody` removal. Effort: 5min.

### Decompression Coverage Gaps (P1)

7. **Test `limitedReadCloser.Close()` error path** — inject a failing ReadCloser, verify the `fmt.Errorf` wrapping. Effort: 10min.
8. **Test `limitedReadCloser.Read()` non-EOF error path** — inject a reader that fails mid-stream. Effort: 10min.
9. **Test `Decompression` encoding-filter reject path** (78.1% → higher) — send an encoding not in the allowed list. Effort: 10min.
10. **ETag + Decompression interaction test** — decompressed body should get ETag based on decompressed content, not compressed bytes. `chain_test.go`. Effort: 45min.
11. **Benchmark decompression** — gzip + deflate + passthrough. `decompression_test.go`. Effort: 15min.
12. **Fuzz test for decompression** — random compressed bodies, malformed data, edge-case sizes. `decompression_test.go`. Effort: 30min.

### ETag Cross-Middleware Tests (P1)

13. **ETag + CORS interaction** — does CORS `Vary: Origin` header affect ETag caching? `chain_test.go`. Effort: 20min.
14. **ETag + Recovery interaction** — does panic recovery bypass ETag generation? `chain_test.go`. Effort: 20min.
15. **ETag + ServerTiming interaction** — verify Server-Timing header coexists with ETag. `chain_test.go`. Effort: 15min.
16. **ETag + MaxBodySize interaction** — verify body-size limit doesn't interfere with ETag buffering. `chain_test.go`. Effort: 15min.
17. **ETag + SecurityHeaders interaction** — verify security headers are present alongside ETag. `chain_test.go`. Effort: 15min.

### Testing Hardening (P2)

18. **Fuzz test for `parseETagList`** — focused quote/comma/backslash/escape combinations. `etag_test.go`. Effort: 30min.
19. **Fuzz test for `stripWeakPrefix`** — ensure no panic on `W/`, `W`, empty, backslash-only. `etag_test.go`. Effort: 15min.
...
22. **Fuzz ETag + Compression together** — longer `fuzztime=30s` to surface deep parser bugs. Effort: 30min.

### Documentation Polish (P2)

23. **Add `nix run .#vulncheck` to RELEASE.md** — document the vulncheck app in the release runbook. Effort: 10min.
...
28. **Upgrade 10-32 and 11-26 to per-item strikethrough** — 90 items total, section-level markers are pragmatic but not fully compliant. Effort: 1hr.
29. **Annotate the 08-07 05:10 report** (prior session) — its items are partially resolved by this session. Inline markers needed. Effort: 20min.

### v1.0 Preparation (P3)

30. **Audit all `Validate()` methods for completeness** — ensure every config struct has one. Effort: 30min.
31. **Review `docs/v1-stability.md`** — classify every exported symbol, verify none missing. Effort: 30min.
32. **Add `ServerConfig.TLSConfig` validation** — `server.go`. Effort: 30min.
33. **Remove deprecated `TokenBucketLimiter`** — or confirm v1.0 timeline for removal. Effort: 1hr.
34. **Evaluate `MiddlewareDecompression` constant** — add to `MiddlewareStack` name constants. `stack.go`. Effort: 10min.

### Code Quality (P3)

35. **Consider `ErrCodeETagComputeFailed`** — for custom `HashFunc` failures. `errors.go`, `etag.go`. Effort: 30min.
36. **Review `etagWriter.isCacheableStatus()`** — `status == 0` means "no WriteHeader called yet". Is ETag generation correct for this case? Effort: 15min.
...
41. **Decide: `If-Match` / `If-Unmodified-Since` support** — 412 Precondition Failed for PUT/DELETE/PATCH. In ROADMAP.md.
42. **Decide: `SkipIfPresent bool` config** — preserve handler-set ETags. In ROADMAP.md.
43. **Decide: `Last-Modified` + `If-Modified-Since` suite** — timestamp-based half of RFC 7232. In ROADMAP.md.
44. **Decide: `If-Range` support** — partial-content (206) responses. In ROADMAP.md.

### Ecosystem (P3)

45. **Study Fiber's ETag middleware** — skip logic for SSE, non-200, empty body. Effort: 30min.
46. **Study `blizzy78/conditional-http`** — If-Match/If-Modified-Since patterns. Effort: 15min.
...
49. **Consider extracting entity-tag parsing into `entitytag` subpackage** — only if If-Match support is added.
