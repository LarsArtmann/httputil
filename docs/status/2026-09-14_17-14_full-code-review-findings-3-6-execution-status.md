...
| ~~B5~~ | ~~**Canonical gate coverage**~~ done — closed 2026-09-14 — nix flake check green and buildflow --build-mode dev run (18-14 report A9/A10) | ~~All documented Go gates run manually and green; `nix fmt` ran (treefmt clean)~~ | ~~`nix flake check` (treefmt verification, server-timing standalone build) and a full `buildflow --build-mode dev` run were **not** executed — manual gates only~~ |
...
~~1. **CSRF security-degrading config: log-only vs remediate** (standing owner question ③; would overturn DECISION_LOG 2026-08-08).~~ done (resolved 2026-09-15 — owner ruling; design pass + B1 fallback shipped in v1.2.0)
...
~~5. **File the verified go-error-family issue** — TODO marks it "Owner action (own repo)"; draft is saved and verified.~~ done (filed 2026-09-15 as go-error-family#5)
...
20. ~~Consider renaming/rewording the FEATURES "New middleware" coverage header (it predates the taxonomy rename and mixes sections).~~ done (done 2026-09-23 — FEATURES coverage section renamed to Middleware and server internals)
...
50. ~~Next docs-health VERIFY pass should re-check today's coverage numbers (97.2/98.6/97.5) against a fresh race-profile run.~~ done (docs-health pass 2026-09-23, re-measured — 97.5% httputil / 99.1% httpspec; FEATURES refreshed)
