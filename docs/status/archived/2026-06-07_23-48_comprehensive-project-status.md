...
1. ~~**Keep the constraint** — document "gzip only" as an intentional limitation. Users who need brotli use a different middleware.~~ done (chosen & documented: constraint kept; brotli via WriterFactory docs)
2. ~~**Relax the constraint** for a zero-transitive-dep brotli library like `github.com/andybalholm/brotli`. This adds one more dependency but delivers real performance gains.~~ done (Won't implement — dependency policy; WriterFactory plugin interface shipped instead)
3. ~~**Add a `WriterFactory` plugin interface** — `CompressionConfig.WriterFactory func(io.Writer) io.WriteCloser`. Default uses stdlib gzip. Users inject brotli if they want it. Keeps core dependency-free but adds API complexity.~~ done (shipped (WriterFactory plugin interface))
