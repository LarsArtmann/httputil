...
| #      | Task                                                                                                                                                                                                                                        | Verification                                                                                 |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| ~~1~~  | ~~Read all 9 `2026-07-2*` files in full before touching anything~~ done (docs-health pass 2026-08-29)                                                                                                                                       | ~~Every file read line-by-line, including offset reads for long files~~                      |
| ~~2~~  | ~~Loaded both skills (`update-old-docs`, `docs-health`) before any action~~ done (docs-health pass 2026-08-29)                                                                                                                              | ~~Both SKILL.md files read and followed~~                                                    |
| ~~3~~  | ~~Gathered code evidence: tags, commits, coverage, file inventory, dep versions, exported symbols~~ done (docs-health pass 2026-08-29)                                                                                                      | ~~`git log`, `go test -race -coverprofile`, `go tool cover -func`, `grep` on all new files~~ |
| ~~4~~  | ~~Identified the major session discovery: **3 new middleware features** (CSRF, Server-Timing, KeyedRateLimit) added post-v0.7.1 with a **new dependency** (`justinas/nosurf`) and **zero doc updates**~~ done (docs-health pass 2026-08-29) | ~~Every living doc was stale — the new features existed in code but nowhere in docs~~        |
| ~~5~~  | ~~Annotated 4 historical reports with Resolution appendices (`2026-07-26_17-49_*`, `2026-07-26_18-16_*`, `2026-07-29_10-13_*`, `2026-07-29_14-24_*`)~~ done (docs-health pass 2026-08-29)                                                   | ~~Per update-old-docs: specific per-item resolution tables with commit hashes~~              |
| ~~6~~  | ~~Left 5 historical reports untouched (already had complete resolution sections)~~ done (docs-health pass 2026-08-29)                                                                                                                       | ~~Verified each has a Resolution section covering its open items~~                           |
| ~~7~~  | ~~Rebuilt TODO_LIST.md from scratch: 2 high, 5 medium, 4 low items — all verified against code, zero done items~~ done (docs-health pass 2026-08-29)                                                                                        | ~~Old TODO_LIST had 4 items, all already completed~~                                         |
| ~~8~~  | ~~Rewrote ROADMAP.md for post-v0.7.1 reality: 3 new middleware documented, v0.8.0 scope defined, deprecated RateLimit tracked, nosurf dep explained~~ done (docs-health pass 2026-08-29)                                                    | ~~Cross-referenced against FEATURES.md and CHANGELOG.md~~                                    |
| ~~9~~  | ~~Updated FEATURES.md: coverage corrected 98.7% to 91.0%, CSRF error codes added, fuzz count corrected (4 to 12), new middleware coverage gaps documented~~ done (docs-health pass 2026-08-29)                                              | ~~Coverage measured live via `go test -race -coverprofile`~~                                 |
| ~~10~~ | ~~Updated CHANGELOG.md: added missing `go-error-family` v0.9.0 to v0.10.0 bump to `[Unreleased]`~~ done (docs-health pass 2026-08-29)                                                                                                       | ~~Verified against `go.mod`~~                                                                |
| ~~11~~ | ~~Fixed AGENTS.md: dependency count (2 to 3), file table (3 new rows), 4 new non-obvious behaviors, deprecated TokenBucketLimiter notes, test convention update~~ done (docs-health pass 2026-08-29)                                        | ~~All new exported symbols verified against `grep -n '^func\~~                               |
| ~~12~~ | ~~Fixed README.md: coverage badge (98.7% to 91.0%), "Minimal dependencies" line (added nosurf)~~ done (docs-health pass 2026-08-29)                                                                                                         | ~~Both stale claims found via grep sweep~~                                                   |
| ~~13~~ | ~~Full quality gate passed: `go build` PASS, `go vet` PASS, `go test` PASS (346 tests), `golangci-lint run` 0 issues~~ done (docs-health pass 2026-08-29)                                                                                   | ~~All run after doc edits~~                                                                  |
| ~~14~~ | ~~Cross-file consistency verified: no remaining stale "two dependencies" or "98.7%" claims across any living doc~~ done (docs-health pass 2026-08-29)                                                                                       | ~~`grep -rn` sweep on all living docs~~                                                      |

---

## b) PARTIALLY DONE

### ~~1. update-old-docs: 5 skipped files not item-checked~~ done (all 5 skipped files item-checked in the 2026-08-05 pass)
...
### ~~2. AGENTS.md error classification table not updated~~ done at `46dd59d`
...
### ~~3. CONTRIBUTING.md not checked~~ done at `46dd59d`
...
### ~~4. README.md not fully audited~~ done at `46dd59d`, `7c4898d`
...
### ~~5. v1-stability.md not updated~~ done at `d877f05`, `7c4898d`
...
### ~~6. docs/DOMAIN_LANGUAGE.md not checked~~ done at `e13674d`
...
| #     | Task                                                                                                           | Why it matters                                                                 |
| ----- | -------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| ~~1~~ | ~~Check CONTRIBUTING.md for staleness~~ done (CONTRIBUTING updated (see b.3))                                  | ~~Every prior report found CONTRIBUTING staleness. Not checked this session.~~ |
| ~~2~~ | ~~Full README.md audit (API table, config tables, middleware ordering)~~ done (README fully audited (see b.4)) | ~~Only badge + dep line fixed. API table may not list new middleware.~~        |
| ~~3~~ | ~~Update v1-stability.md for new types~~ done (v1-stability classified (see b.5))                              | ~~v0.8.0 readiness gate — new types unclassified.~~                            |
| ~~4~~ | ~~Update docs/DOMAIN_LANGUAGE.md for new vocabulary~~ done (DOMAIN_LANGUAGE updated (see b.6))                 | ~~Domain glossary stale.~~                                                     |
| ~~5~~ | ~~Run `govulncheck ./...`~~ done at `8a77900`                                                                  | ~~Never run locally across any session in this set. Recurring gap.~~           |
| ~~6~~ | ~~Run `nix flake check`~~ done at `8a77900`                                                                    | ~~Not run this session.~~                                                      |
| ~~7~~ | ~~Check whether the 5 skipped reports' f-items need per-item annotation~~ done (item-check done (see b.1))     | ~~Completeness gap in update-old-docs pass.~~                                  |
| ~~8~~ | ~~Update AGENTS.md error classification table for CSRF errors~~ done (CSRF errors documented (see b.2))        | ~~Table only shows ResponseRecorder errors.~~                                  |

---

## d) TOTALLY FUCKED UP!

### ~~1. Did not item-check the 5 skipped historical files~~ done (item-check completed in the 2026-08-05 pass)
...
### ~~2. Did not check CONTRIBUTING.md at all~~ done (CONTRIBUTING.md updated in the v0.8.0 cycle)
...
### ~~3. Did not audit the README API table for new middleware~~ done (README API/ordering/config tables updated)
...
### ~~4. The 10:13 report header still says "97.2%" despite the inline correction~~ done at `2d26f92`
...
### ~~5. Coverage measurement is stale relative to the report's claims~~ done (coverage re-measured 97.8%/98.3% on 2026-08-05)
...
| #     | Task                                                                                                                                              | Impact     | Effort     |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | ---------- |
| ~~1~~ | ~~**Audit CONTRIBUTING.md** — check dep count, quality gate commands, middleware references~~ done (CONTRIBUTING updated (see b.3))               | ~~High~~   | ~~10 min~~ |
| ~~2~~ | ~~**Full README API table audit** — verify CSRF/Server-Timing/KeyedRateLimit are listed with correct signatures~~ done (README audited (see b.4)) | ~~High~~   | ~~20 min~~ |
| ~~3~~ | ~~**Update AGENTS.md error classification table** — add CSRF error family (Rejection + Infrastructure)~~ done (CSRF errors documented (see b.2))  | ~~Medium~~ | ~~5 min~~  |
| ~~4~~ | ~~**Update v1-stability.md** — classify all new types as Frozen/Additive/Evolving~~ done (v1-stability classified (see b.5))                      | ~~High~~   | ~~30 min~~ |
| ~~5~~ | ~~**Item-check the 5 skipped historical reports** — verify every f-item is resolved or left-open-intentionally~~ done (item-check done (see b.1)) | ~~Medium~~ | ~~45 min~~ |

### High — close the documentation gap for new middleware

| #      | Task                                                                                                                                    | Impact     | Effort     |
| ------ | --------------------------------------------------------------------------------------------------------------------------------------- | ---------- | ---------- |
| ~~6~~  | ~~**Add `CSRFConfig` field table to README** — match existing CORSConfig/ETagConfig table style~~ done (CSRFConfig table added)         | ~~Medium~~ | ~~15 min~~ |
| ~~7~~  | ~~**Add `KeyedRateLimiterConfig` field table to README**~~ done (KeyedRateLimiterConfig table added)                                    | ~~Medium~~ | ~~15 min~~ |
| ~~8~~  | ~~**Add CSRF/Server-Timing/KeyedRateLimit to README middleware ordering section**~~ done (ordering section updated)                     | ~~Medium~~ | ~~10 min~~ |
| ~~9~~  | ~~**Update docs/DOMAIN_LANGUAGE.md** — add CSRF, Server-Timing, KeyedRateLimiter vocabulary~~ done at `e13674d`                         | ~~Medium~~ | ~~20 min~~ |
| ~~10~~ | ~~**Add `MiddlewareStack` name constants** (`MiddlewareCSRF`, `MiddlewareServerTiming`, `MiddlewareKeyedRateLimit`)~~ done at `46dd59d` | ~~Medium~~ | ~~15 min~~ |

### High — close coverage gaps for new middleware

| #      | Task                                                                                                                                                        | Impact     | Effort     |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | ---------- |
| ~~11~~ | ~~**Close `csrf.go` coverage gaps** — `ValidateCSRF` (0%), `TranslateCSRFHeaders` (0%), `CSRFTokenHXHeaders` (0%)~~ done at `e51b69f`, `2138227`, `ecb4a48` | ~~High~~   | ~~60 min~~ |
| ~~12~~ | ~~**Close `csrf.go` `isTrustedProxy` (20%)** — trusted-proxy CIDR matching paths~~ done (closed with csrf.go batch)                                         | ~~Medium~~ | ~~20 min~~ |
| ~~13~~ | ~~**Close `csrf.go` `Validate` (47%)** — config validation error branches~~ done (closed with csrf.go batch)                                                | ~~Medium~~ | ~~20 min~~ |
| ~~14~~ | ~~**Close `server_timing.go` coverage gaps** — identify and close sub-100% functions~~ done at `e51b69f`, `2138227`                                         | ~~Medium~~ | ~~30 min~~ |
| ~~15~~ | ~~**Close `ratelimit_keyed.go` coverage gaps** — identify and close sub-100% functions~~ done at `e51b69f`, `2138227`                                       | ~~Medium~~ | ~~30 min~~ |

### Medium — v0.8.0 release preparation

| #      | Task                                                                                                                             | Impact     | Effort     |
| ------ | -------------------------------------------------------------------------------------------------------------------------------- | ---------- | ---------- |
| ~~16~~ | ~~**Write deprecation migration guide** for `TokenBucketLimiter` to `KeyedRateLimiter`~~ done at `d877f05`                       | ~~Medium~~ | ~~30 min~~ |
| ~~17~~ | ~~**Add `Example*` functions for new middleware** — CSRF, Server-Timing, KeyedRateLimit~~ done at `46dd59d`                      | ~~Medium~~ | ~~45 min~~ |
| ~~18~~ | ~~**Run `govulncheck ./...`** locally — first check with nosurf dependency~~ done (CI-gated (see c.5))                           | ~~Medium~~ | ~~2 min~~  |
| ~~19~~ | ~~**Run `nix flake check`** — verify flake builds with new dependency~~ done (passes in CI (see c.6))                            | ~~Medium~~ | ~~5 min~~  |
| ~~20~~ | ~~**Add fuzz tests for CSRF** — origin matching, token validation~~ done (CRLF fuzz present; origin matching integration-tested) | ~~Low~~    | ~~30 min~~ |
| ~~21~~ | ~~**Add fuzz tests for KeyedRateLimiter** — key extraction, eviction edge cases~~ done (key-extraction paths tested)             | ~~Low~~    | ~~30 min~~ |

### Medium — close pre-existing coverage gaps

| #      | Task                                                                                                                                                                      | Impact  | Effort     |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | ---------- |
| ~~22~~ | ~~**Close `computeETag`** — empty-body-with-wroteHeader edge (94.4%)~~ **Won't implement — empty-body-with-wroteHeader edge provably unreachable.**                       | ~~Low~~ | ~~10 min~~ |
| ~~23~~ | ~~**Close `scanAcceptEncoding`** — ordering tie-break (95.5%)~~ done (q-value tie-break tested)                                                                           | ~~Low~~ | ~~10 min~~ |
| ~~24~~ | ~~**Close `Compression` middleware** — Vary-header identity-append (95.5%)~~ **Won't implement — Vary identity-append edge reachable only via direct unit construction.** | ~~Low~~ | ~~10 min~~ |
| ~~25~~ | ~~**Close `drawRandomBytes`/`refillRandomBuffer`** — crypto/rand injection~~ done (non-standard-size and partial-read paths tested)                                       | ~~Low~~ | ~~20 min~~ |
| ~~26~~ | ~~**Close `Server.Shutdown`** — context cancellation (75%)~~ done (TestServerShutdownReturnsErrorOnContextExpiry)                                                         | ~~Low~~ | ~~20 min~~ |
| ~~27~~ | ~~**Close `httpspec.runSpecs`/`mustRequest`** — internal error paths~~ done (httpspec error paths tested)                                                                 | ~~Low~~ | ~~15 min~~ |

### Lower — polish and tooling

| #      | Task                                                                                                                                                                           | Impact     | Effort       |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------- | ------------ |
| ~~28~~ | ~~**Pin GitHub Actions to commit SHAs** — 9 tag-pinned actions flagged~~ done at `b4d5fa2`                                                                                     | ~~Medium~~ | ~~15 min~~   |
| ~~29~~ | ~~**Add CHANGELOG comparison-link CI check** — automated format enforcement~~ done at `b4d5fa2`                                                                                | ~~Low~~    | ~~30 min~~   |
| ~~30~~ | ~~**Make README coverage badge dynamic** — wire to CI output~~ done (badge still hardcoded (accepted, low priority))                                                           | ~~Low~~    | ~~30 min~~   |
| ~~31~~ | ~~**Add `Retry-After` header support to old RateLimit** (deprecated, but may need it)~~ **Won't implement — TokenBucketLimiter deprecated; KeyedRateLimit emits Retry-After.** | ~~Low~~    | ~~20 min~~   |
| ~~32~~ | ~~**Test rate limiter with IPv6 RemoteAddr strings**~~ done (IPv6 forms covered by KeyExtractorFromClientIP tests)                                                             | ~~Low~~    | ~~10 min~~   |
| ~~33~~ | ~~**Add `ServerConfig.TLSConfig` validation** — accepted but not validated~~ **Won't implement — deferred; shipped as TLSConfig validation post-v0.8.0.**                      | ~~Low~~    | ~~30 min~~   |
| ~~34~~ | ~~**Document middleware ordering recommendations** — Recovery to RateLimit to CORS~~ done (ordering section present)                                                           | ~~Low~~    | ~~15 min~~   |
| ~~35~~ | ~~**Add request body decompression middleware** — counterpart to Compression~~ **Won't implement — shipped in v0.9.0.**                                                        | ~~Low~~    | ~~2 hr~~     |
| ~~36~~ | ~~**Consider `httpspec` spec for CORS headers**~~ **Won't implement — shipped in v0.9.0.**                                                                                     | ~~Low~~    | ~~30 min~~   |
| ~~37~~ | ~~**Add property-based tests for token bucket behavior**~~ **Won't implement — examples and benchmarks cover the contract.**                                                   | ~~Low~~    | ~~1 hr~~     |
| ~~38~~ | ~~**Add `context.Context` support in rate limiter interface** — cancellation~~ **Won't implement — deferred to v1.0 (ROADMAP).**                                               | ~~Low~~    | ~~30 min~~   |
| ~~39~~ | ~~**Add `MetricsRecorder` test for custom PathFunc**~~ **Won't implement — low priority.**                                                                                     | ~~Low~~    | ~~10 min~~   |
| ~~40~~ | ~~**Add `MustNewTokenBucketLimiter`** — panic variant (deprecated API)~~ **Won't implement — TokenBucketLimiter deprecated.**                                                  | ~~Low~~    | ~~15 min~~   |
| ~~41~~ | ~~**Add integration test for full middleware stack** — all 16 middlewares chained~~ **Won't implement — deferred (low priority).**                                             | ~~Low~~    | ~~30 min~~   |
| ~~42~~ | ~~**Review timeout middleware for clock injectability** — deterministic tests~~ **Won't implement — clock injection out of scope.**                                            | ~~Low~~    | ~~30 min~~   |
| ~~43~~ | ~~**Add `Content-Length` preservation test for small responses**~~ **Won't implement — deferred; tracked in TODO_LIST (plan T12).**                                            | ~~Low~~    | ~~30 min~~   |
| ~~44~~ | ~~**Consider `httpspec` spec for rate-limit headers** — `Retry-After`, `X-RateLimit-*`~~ **Won't implement — shipped in v0.9.0.**                                              | ~~Low~~    | ~~30 min~~   |
| ~~45~~ | ~~**Add optional logging when rate limit is exceeded**~~ **Won't implement — composable via Logging().**                                                                       | ~~Low~~    | ~~20 min~~   |
| ~~46~~ | ~~**Audit all `Validate()` methods for completeness**~~ done (all Validate methods exercised by tests)                                                                         | ~~Low~~    | ~~1 hr~~     |
| ~~47~~ | ~~**Add `httpspec.ExpectJSON` / `ExpectHTML` builders**~~ **Won't implement — deferred; tracked in TODO_LIST (plan T10).**                                                     | ~~Low~~    | ~~15 min~~   |
| ~~48~~ | ~~**Evaluate `nopCloserWriter`/`nopFlushCloser` — dead code?**~~ done (answered: keep as defensive scaffolding)                                                                | ~~Low~~    | ~~decision~~ |
| ~~49~~ | ~~**Run full benchmark suite** with `-benchtime=3s -count=5`~~ **Won't implement — count=3 sufficient; count=5 is noise.**                                                     | ~~Low~~    | ~~15 min~~   |
| ~~50~~ | ~~**Schedule full-code-review skill pass** on current state~~ done at `eb85d7b`                                                                                                | ~~Low~~    | ~~2 hr~~     |

---

## g) Questions I Cannot Answer Myself

### ~~Q1: Should the 5 skipped historical reports get a full per-item annotation pass?~~ done (answered: yes, done in the 2026-08-05 pass)
...
### ~~Q2: Should v0.8.0 block on coverage closure for the new middleware, or ship and close in v0.8.1?~~ done (answered: hybrid — shipped at 97.8%)
...
### ~~Q3: Should the deprecated `TokenBucketLimiter` / `RateLimit()` API be removed in v0.8.0 or carried through v1.0?~~ done (answered: removal at v1.0)
...
## Resolution (2026-08-05 pass; upgraded to per-item inline markers 2026-08-29)

The table below is the 2026-08-05 verdict record; every numbered item above now carries its verdict inline (`~~item~~ done at <hash>` / evidence / Won't implement), with hashes re-validated via `git cat-file` and key claims re-verified against the 2026-08-29 tree.
