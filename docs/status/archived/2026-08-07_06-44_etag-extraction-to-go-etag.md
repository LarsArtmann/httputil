...
26. ~~**Lint in go-etag** — `golangci-lint run` reports **1 issue**: `unparam` on `newTestRequest` (path parameter always receives `"/"`). This is because the test helper was copied verbatim from httputil where `path` varies across many test suites, but in go-etag all callers pass `"/"`. **Fix: remove the `path` parameter or add a second path value to justify it.**~~ done (go-etag v0.2.0 released lint-clean)
...
28. ~~**docs/research/2026-07-05_httputil-vs-huma.md** — Still has 3 ETag references in the comparison table. This is a historical research document and arguably should stay as-is (it was true at time of writing), but it may confuse readers.~~ **Won't implement — research doc is a point-in-time comparison; the extraction is documented in living docs.**

---

## c) NOT STARTED

29. ~~**No LICENSE file** in go-etag — README says "MIT" but there is no LICENSE file.~~ done (LICENSE exists at go-etag release)
30. ~~**No .gitignore** in go-etag — standard Go `.gitignore` needed.~~ done (repo bootstrapped at release)
31. ~~**No Nix flake** in go-etag — httputil has one; go-etag should match for consistency.~~ done (flake exists)
32. ~~**No GitHub remote** for go-etag — `git remote -v` shows nothing. No GitHub repo created.~~ done (GitHub remote: LarsArtmann/go-etag)
33. ~~**No GitHub Actions CI** for go-etag — httputil has CI; go-etag needs tests+lint+govulncheck.~~ done (CI at release)
34. ~~**No `go.work` integration** — httputil's `go.work` lists `server_timing` but not `go-etag`. If `go-etag` should be developed alongside httputil (local replace), it needs adding. If it's fully independent, this is fine.~~ done (consumed via released go.mod require since v0.12.0 (no workspace entry needed))
35. ~~**No `.editorconfig`** in go-etag.~~ done (bootstrapped)
36. ~~**No `flake.nix` check** run on either project — AGENTS.md says "Check for `flake.nix` first."~~ done (flake checks run)
37. ~~**No `nix flake check`** on go-etag.~~ done (same)
38. ~~**golangci-lint fmt** not run on go-etag — formatting may not match gofumpt/golines@120/gci standards.~~ done (released formatted)
39. ~~**Coverage report** not generated for go-etag — unknown coverage percentage.~~ done (released with its own suite)
...
20. ~~**Update `docs/research/2026-07-05_httputil-vs-huma.md`** — note that ETag has been extracted, or leave as historical.~~ **Won't implement — point-in-time research doc.**
...
29. ~~**Consider extracting `responseWrapper` into a shared module** — it's now duplicated between httputil (compress) and go-etag. A `go-httpwrapper` or similar could eliminate this.~~ **Won't implement — flat-root + ADR 0001: adapter, not shared module.**
...
31. ~~**Consider extracting `RegisterErrorClassifications`** — the stdlib sentinel registration is now duplicated. Could be a shared `go-httperrors` module.~~ **Won't implement — registration stays in httputil (documented superset of go-etag templates).**
32. ~~**Add migration guide** — `docs/migrating-from-httputil-etag.md` for users replacing `httputil.ETag` with `etag.ETag`.~~ **Won't implement — adapter still composes; migration is an import swap documented in CHANGELOG [0.12.0] Deprecated.**
...
42. ~~**Update `docs/planning/` if any plan references ETag** as a future httputil deliverable.~~ done (planning docs annotated by the 2026-08-30 pass)
