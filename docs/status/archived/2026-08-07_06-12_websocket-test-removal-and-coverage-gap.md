...
| ~~f.2~~  | ~~**Decide: restore a real-connection Hijack upgrade test?**~~ **done 2026-08-30** — `TestChain_HijackUpgrade_BytesFlowThroughHijackedConn` in `chain_hijack_test.go` (real server, bidirectional echo, `-race -count=10` stable). The deleted test was the only one proving Content-Encoding/ETag are not injected into a 101 Switching Protocols response and that post-hijack bytes flow uncorrupted. This is a real correctness property of the middleware, not dead weight. If the removal was about the _test style_ (heavy, real-TCP) rather than the _property_, a lighter `httptest.NewServer` + header-only assertion could replace it. | High         | M      | Testing           |
...
~~Open as of 2026-08-29: c.3/f.2/f.7 (restore a real-connection Hijack test — deferred design decision, plus the helper restore contingent on it), f.4 (lighter upgrade-response test — not written), f.6 (test-name/assertion lint convention), f.9 (mutation-test the Hijacker-preservation test).~~ All resolved 2026-08-30 (see inline markers; `chain_hijack_test.go`, AGENTS.md convention section). Sections d)/e) are narrative session facts and process lessons, intentionally unmarked.
