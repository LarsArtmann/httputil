...
1. ~~Decide and implement the fate of `CompressionConfig.QValues`.~~ done (shipped (compression_qvalue.go + property tests))
2. ~~Fix `BenchmarkCompression` memory artifact.~~ done (done (harness fixed 2026-08-30))
3. ~~Add configurable content-type filtering to `CompressionConfig`.~~ done (shipped (IncompressibleTypes))
4. ~~Implement fast path for single-encoding `Accept-Encoding`.~~ done (shipped (singleToken fast path, 0 allocs))
5. ~~Split/refactor `negotiateEncoding` to reduce cyclomatic complexity.~~ done (shipped (negotiator split + lexicographic tie-break))
6. ~~Add tests covering `QValues` behavior once designed.~~ done (shipped (compression_qvalue.go + property tests))
7. ~~Add tests for compression error branches (`Close`, pool type mismatch, custom factory without `Reset`).~~ done (shipped (compress_writer_test.go error-branch tests))
8. ~~Add deflate-specific benchmark.~~ done (shipped (DefaultWriterFactories))
9. ~~Add `parseQValue` fuzz tests.~~ done (shipped (compression_qvalue.go + property tests))
10. ~~Add `WriterFactory` examples to `example_test.go`.~~ done (done (example_test + brotli-zstd guide))
11. ~~Add RequestID generator benchmark.~~ done (done (BenchmarkRequestID))
12. ~~Refactor `id_generator.go` into an injectable generator type.~~ done (done (RequestIDConfig.GenerateID is injectable))
13. ~~Resolve gopls vs. `golangci-lint` diagnostic discrepancy.~~ done (moot (LSP hygiene note; toolchain gates are authoritative))
14. ~~Format `README.md` API tables.~~ done (done (markdown tables normalized))
15. ~~Improve overall test coverage to 95%+.~~ done (done (97.0% today))
16. ~~Add CORS origin map/trie optimization.~~ done (parked in ROADMAP legacy-brainstorm line (2026-08-30))
17. ~~Pre-join CORS config slices at validation time.~~ done (parked in ROADMAP legacy-brainstorm line (2026-08-30))
18. ~~Implement `MiddlewareStack` with ordering validation.~~ done (shipped (stack.go))
19. ~~Add `ResponseWriter` capability interface.~~ done (shipped (DetectCapabilities, capabilities.go))
20. ~~Evaluate streaming ETag with rolling hash.~~ done (Won't implement — ROADMAP Non-goals: headers precede body, buffering is mandatory)
21. ~~Add request/response metrics middleware.~~ done (shipped (metrics.go))
22. ~~Add rate-limiting middleware.~~ done (shipped (ratelimit.go, deprecated; KeyedRateLimiter succeeded it))
23. ~~Add request body size limit middleware.~~ done (shipped (maxbodysize.go))
24. ~~Add WebSocket upgrade integration test through Compression + ETag.~~ done (Won't implement — removed 2026-08-07 as fragile; Hijack tiers restored 2026-08-30)
25. ~~Automate benchmark regression checks in CI.~~ done (parked in ROADMAP legacy-brainstorm line (2026-08-30))
