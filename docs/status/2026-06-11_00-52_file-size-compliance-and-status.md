...
1. **Make content-type filtering configurable via `CompressionConfig`** — currently hardcoded `incompressiblePrefixes` in `compress_writer.go`
2. **Add `MiddlewareStack` type with ordering validation** — enforce correct middleware ordering (e.g., ETag inside Compression)
3. **Add `ResponseWriter` capability interface** for Hijack/Flush — unify detection across wrappers
4. **Implement deflate support** using `compress/flate` — second encoding option
5. **Add `Accept-Encoding` quality value parsing** per RFC 7231 — proper content negotiation
6. **Evaluate streaming ETag option** using rolling hash — avoid buffering entire response
7. **Consider request/response metrics middleware** — expvar or custom histograms
8. **Consider rate-limiting middleware** — sliding window or token bucket
9. **Consider request body size limit middleware**
...
1. **Update `FEATURES.md`** — fix coverage (92.4%), test count (148), mark completed items
2. **Update `TODO_LIST.md`** — verify against code, update date
3. **Test `compressWriter.Flush` while-buffering path** — currently at 61.5%, the most complex state machine
4. **Test `startCompressAndStream` error branch** — gzip write failure during streaming (66.7%)
5. **Test `flushPlainAndStream` error branches** — buffered write failure, plain write failure (76.9%)
6. **Test `writePlain` error branch** — ResponseWriter.Write failure (75%)
7. **Test `writeCompressed` error branch** — gzipWriter.Write failure (75%)
8. **Test `etagWriter.Flush` error branches** — computeETag failure, write failure (77.8%)

### Tier 2: Quick Wins (High impact, low effort)

9. **Adopt `sivchari/govalid`** for struct validation — replace manual `Validate()` methods with generated validation
10. **Make content-type filtering configurable** in `CompressionConfig` — add `IncompressibleTypes []string` field
11. **Add `MiddlewareStack` builder** — `NewStack().Add(Compression(cfg)).Add(ETag(cfg)).Build()` with ordering validation
12. **Add `ResponseWriter` capability interface** — `type CapableWriter interface { Hijack() bool; Flush() bool }` or similar

### Tier 3: Feature Work (Medium effort)

13. **Implement deflate support** — `compress/flate` is stdlib, add as secondary encoding option
14. **Accept-Encoding quality value parsing** — proper RFC 7231 content negotiation
15. **Add request body size limit middleware** — `MaxBodySize(int64) Middleware`
16. **Add rate-limiting middleware** — sliding window or token bucket
17. **Add request/response metrics middleware** — optional, expvar or custom histograms

### Tier 4: Polish & Hardening

18. **Add integration tests for all middleware pairs** — every combination through Chain()
19. **Add HTTP/2 integration tests** — verify middlewares work with HTTP/2 semantics
20. **Add WebSocket upgrade integration tests** — full lifecycle through Compression + ETag
21. **Benchmark with real-world payload sizes** — 1KB, 10KB, 100KB, 1MB response bodies
22. **Profile and optimize gzip pool** — verify pool hit rate, check for GC pressure
23. **Add `go fuzz` corpus seeds** — expand fuzz test coverage with real-world patterns
24. **Evaluate streaming ETag** — rolling hash prototype, benchmark memory savings
25. **Consider Brotli support** — blocked by single-dependency policy, but `WriterFactory` interface could solve it
...
1. **Add `govalid` as the 2nd allowed dependency** — same author trust model, zero transitive deps
2. **Keep manual `Validate()` methods** — simpler, no new deps, but more boilerplate
3. **Change policy to allow "zero-transitive-dep" libraries** — opens door to carefully vetted tools
