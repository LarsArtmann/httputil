...
1. **M1 — Verification commands:** Ran `govulncheck` (no vulnerabilities), `nix flake check` (all passed), `go mod verify` (all verified), `golangci-lint fmt` (clean). All 6 commands that prior sessions lied about are now independently verified.
2. **M1 — Spec count verified:** 18 standard + 7 extra (4 CORS + 3 RateLimit) = 25 total. Updated FEATURES.md.
...
4. **M3 — Doc claims verified against source:** `git diff v0.7.1..v0.8.0 --stat` cross-checked. Benchmark count (41), fuzz count (20), example count (23), Validate methods (11) — all confirmed.
5. **M4 — MaxBodySize validation:** New `MaxBodySizeConfig` struct with `Validate()`, `DefaultMaxBodySizeConfig()` (1 MiB), `MaxBodySizeMiddleware(cfg)`. 4 new tests. Existing `MaxBodySize(maxBytes)` preserved for backward compatibility.
6. **M4 — ShutdownTimeout validation:** New `ServerConfig.ShutdownTimeout` field (default 30s), `Validate()` rejects negative, `Server.Shutdown(ctx)` auto-derives timeout when context has no deadline. 2 new tests.
7. **M5 — Footgun warnings:** `KeyExtractor` type doc warns about empty-return disabling rate limiting. AGENTS.md architecture table updated (KeyedRateLimiterConfig gained `Validate()`, MaxBodySize gained config struct). canonicalheader note already existed (verified).
8. **M6 — Coverage gaps closed:** All 5 `cors_ratelimit_specs.go` functions improved from 80-91% to 100%. httpspec coverage: 96.0% → 99.3%. 7 new test functions covering invalid credentials, missing Retry-After, negative Retry-After, invalid hint headers, wildcard origin Vary pass-through, invalid reset header.
9. **M7 — CI race detection:** Added `go test -race -count=10` stress test step to `.github/workflows/ci.yml`.
10. **M8 — Coverage regression gate:** Added coverage threshold check (fail if < 95%) to CI via `go tool cover -func` + awk parsing.
11. **M9 — CHANGELOG freeze policy:** Decided autonomously: freeze at tag. Documented in AGENTS.md under new "CHANGELOG Freeze Policy" section.
12. **M10 — Health + Metrics benchmarks:** 6 new benchmarks: `BenchmarkHealthHandler`, `BenchmarkLiveHandler`, `BenchmarkReadyHandler`, `BenchmarkMetricsMiddleware` (default, with body, with custom path).
13. **M11 — httpspec benchmark modernization:** Migrated `httpspec/benchmark_test.go` from `b.N` to `b.Loop()`.
14. **M12 — Fuzz tests:** Added `FuzzETagConditional` (If-Match/If-None-Match edge cases) and `FuzzCompressWriterState` (encoding/body/contentType variations). Both verified with `-fuzztime=15s`.
15. **M13 — README Quality Gates:** Added full Quality Gates table (8 gates with commands and status). Fixed duplicate-bracket badge artifact. Updated coverage badge.
16. **M14 — Pre-commit hook:** Created `scripts/pre-commit.sh` (runs `golangci-lint run`), installed in `.git/hooks/pre-commit`.
17. **M15 — AGENTS.md process docs:** Added auto-commit daemon documentation and doc-freshness cadence recommendation.
18. **M16 — Race stress test:** `go test -race -count=100 ./...` — 100/100 PASS.
19. **M17 — Self-review:** Found and fixed ROADMAP coverage split brain (97.8%/96.0% → 97.6%/99.3%).
20. **M19 — D2 diagram:** Generated updated `2026-08-05_httputil-current.d2` + `.svg` reflecting 16-middleware architecture + 3 external deps.
21. **M21 — Decompression middleware:** Full implementation of gzip/deflate request body decompression with `DecompressionConfig`, `Validate()`, bomb protection (`MaxDecompressionSize`), and encoding filter. 7 tests.

---

## b) PARTIALLY DONE

1. **M18 — Domain language cross-reference:** Added `MaxBodySizeConfig`, `MaxBodySizeMiddleware()`, `ServerConfig.ShutdownTimeout` to DOMAIN_LANGUAGE.md. **But:** Did NOT add decompression vocabulary (`DecompressionConfig`, `Decompression()`, `MaxDecompressionSize`, decompression bomb protection). The feature shipped but is invisible in the domain language.
...
3. **M22 — v1.0 cleanup prep:** Updated ROADMAP to reflect decompression shipped. Evaluated TokenBucketLimiter removal scope (self-contained in `ratelimit.go`, clean). **But:** Did not design `context.Context` support or `TLSConfig` validation — just noted they're deferred.

---

## c) NOT STARTED

1. **Example function for `Decompression()`:** Every other major middleware has an `Example*` function (required by `testableexamples` linter). Decompression has none. The linter didn't catch it because it only flags functions that ALREADY have examples, not missing ones — but it's an obvious gap for a new public middleware.
2. **Fuzz test for decompression:** Every other middleware that processes untrusted input has fuzz tests. Decompression processes compressed request bodies (a classic attack surface for zip bombs). No fuzz coverage.
3. **Benchmark for decompression:** I added benchmarks for Health and Metrics but not for Decompression, despite it being a performance-critical middleware (decompression runs on every request with Content-Encoding).
...
6. **CI YAML validation (F7.3):** The plan called for verifying the CI YAML is valid. I did not validate it.
...
| 2   | **Update sub-100% count** from 14 to 18 in FEATURES.md (3 new decompression.go functions) | Critical | 2 min  |
| 3   | **Update middleware count** from 16 to 17 everywhere (FEATURES, ROADMAP, README)          | Critical | 5 min  |
| 4   | **Add decompression.go to FEATURES.md sub-100% list** with actual percentages             | Critical | 5 min  |
...
| #   | Task                                                                                | Impact | Effort |
| --- | ----------------------------------------------------------------------------------- | ------ | ------ |
| 6   | **Write test for `limitedReader.Close()`** (currently 0%)                           | High   | 5 min  |
| 7   | **Write test for `limitedReader.Read()` error path** (currently 58.3%)              | High   | 10 min |
| 8   | **Write test for decompression bomb protection** (exceed MaxDecompressionSize)      | High   | 10 min |
| 9   | **Write test for `Decompression()` encoding filter** (rejecting unallowed encoding) | High   | 5 min  |
| 10  | **Write `ExampleDecompression`** function (consistency with all other middleware)   | Medium | 10 min |
...
| #   | Task                                                                                | Impact | Effort |
| --- | ----------------------------------------------------------------------------------- | ------ | ------ |
| 15  | **Add `DecompressionConfig` to DOMAIN_LANGUAGE.md**                                 | Medium | 5 min  |
| 16  | **Write `BenchmarkDecompression`** (gzip + deflate + passthrough)                   | Medium | 15 min |
| 17  | **Write `FuzzDecompression`** (random compressed bodies)                            | Medium | 20 min |
| 18  | **Add `testdata/fuzz/` to `.gitignore`**                                            | Low    | 2 min  |
| 19  | **Update AGENTS.md middleware table** with decompression.go (already added, verify) | Low    | 2 min  |
...
| 21  | **Update `docs/v1-stability.md`** to classify Decompression (Frozen/Additive/Evolving) | Medium | 10 min |
...
| #   | Task                                                                                      | Impact | Effort   |
| --- | ----------------------------------------------------------------------------------------- | ------ | -------- |
| 24  | **v0.9.0 milestone is now empty** — decompression was the only planned v0.9.0 feature     | Medium | decision |
| 25  | **Evaluate whether Decompression should be in the `MiddlewareStack` name constants**      | Low    | 10 min   |
| 26  | **Add decompression to `docs/migrating-to-keyed-rate-limiter.md`** — N/A, but check docs/ | Low    | 5 min    |
| 27  | **Run `art-dupl` to verify decompression doesn't introduce duplication**                  | Low    | 5 min    |
...
| 33  | **Establish recurring doc-freshness cadence** (monthly docs-health pass)                  | Low    | 5 min    |
...
| 38  | **Verify `art-dupl` "0 clones" claim** is still true after decompression.go               | Low    | 5 min    |
