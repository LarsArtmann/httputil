...
1. **Fix `FEATURES.md` test count** — Change "193 tests" to actual count (158). Stop hardcoding.
2. **Fix `FEATURES.md` coverage numbers** — Remove stale 91.2%/90.4%, use 90.2%.
3. **Fix `TODO_LIST.md:53`** — Change "112 tests, 91.2%" to current numbers.
4. **Fix `TODO_LIST.md:33`** — Remove "Itoa, Join" from benchmark list (deleted).
5. **Add `health.go` to `AGENTS.md` architecture table** — exports, purpose.
6. **Add `server.go` to `AGENTS.md` architecture table** — exports, purpose.
7. **Add Server + Health sections to `FEATURES.md`** — they're shipped features.
8. **Update `CHANGELOG.md` [Unreleased]** — Add util.go removal entry.
9. **Remove `CHANGELOG.md` stale "193 tests"** — Or mark it clearly as a historical snapshot.

### Code Quality (Medium Effort, High Impact)

10. **Make content-type filtering configurable** — Currently hardcoded deny-list in `compress_content_type.go`. Add to `CompressionConfig`.
11. **Add `MiddlewareStack` type** — Validate ordering at construction (e.g., Recovery outermost, Compression before ETag).
12. **Add `ResponseWriter` capability interface** — Unify Hijack/Flush detection across wrappers.
13. **Add compression error branch tests** — Cover `startCompression` type mismatch, `Close` errors.
14. **Add CORS wildcard fuzz cases** — Unusual patterns (`*.*`, `.*`, `a.*.b`).
15. **Add `ResponseRecorder` hijack failure test** — Underlying writer doesn't implement Hijacker.

### Architecture Improvements (Higher Effort)

16. **Pre-compute CORS header strings at config time** — `strings.Join(cfg.AllowedMethods, ", ")` runs on every request; could be computed once in `CORS()`.
17. **Fast path for single-encoding `Accept-Encoding`** — Skip q-value parsing when header is just `gzip`.
18. **Evaluate incremental ETag hashing** — Compute CRC32 during `Write` calls instead of on flush.
19. **Add streaming ETag option** — Rolling hash for large responses (>1MB).

### New Features (Higher Effort, Lower Priority)

20. **Request body size limit middleware** — Simple, high value for API protection.
21. **Rate-limiting middleware** — Token bucket or sliding window.
22. **Request/response metrics middleware** — Optional, via `expvar` or custom histograms.
23. **Document brotli/zstd factory examples** — Show how to wire custom `WriterFactory` implementations.
24. **Add `Server` graceful shutdown integration test** — Verify `Start()` + `Shutdown()` lifecycle.
25. **Add `HealthHandler` configurable readiness checker** — Allow custom readiness function instead of always `"up"`.
