...
2. **Legacy CSRF code migration:** keep `csrf_samesite_insecure` (underscore, breaks `InDomain(DomainCSRF)`) for backward compatibility, or migrate to `csrf.samesite_insecure` now while pre-v1? Both are defensible; the tradeoff (compat vs. hierarchy consistency) is a product call.
3. **Exported domain constants now or at v1?** Adding `DomainCORS`, `DomainServer`, ... is new public API surface in v0.x. Add now (consumers get compile-safe domains immediately) or hold until the v1 API freeze?
