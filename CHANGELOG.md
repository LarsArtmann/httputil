...
- **CSRF `Sec-Fetch-Site` attestation-conflict defense** (`csrf.go`): the full-code-review trust-model question was verified against the pinned nosurf v1.2.0 source — `ensureSameOrigin` short-circuits ALL Origin/Referer validation on a literal `Sec-Fetch-Site: same-origin` header. Browsers cannot forge it (forbidden header name), but any non-browser client can. Requests whose attestation is contradicted by an `Origin` header that nosurf itself would reject (cross-origin, untrusted, or unparseable) are now rejected with the new `ErrCSRFAttestationConflict` (`csrf.origin_attestation_conflict`, Rejection) before nosurf sees them; the `null` origin stays exempt. The trust boundary is documented on `CSRFMiddleware`, `SetPlaintextHTTPOrigin`, and in the README. Six new tests including a probe that pins the documented nosurf short-circuit; run 5x under `-race`.
