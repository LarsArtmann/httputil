...
- **Clone-detection baseline is now clean at `-t 2`** (`compression_negotiator.go`, `csrf.go`, `server_timing/server_timing.go`): the three production micro-pairs art-dupl reported at threshold 2 are extracted. The Accept-Encoding scan loops share one `advanceWhile` skeleton with named `isEntrySeparator`/`isEntryContent` predicates (0 allocs/op on the negotiator benchmarks); the `withParsedTrustedProxies` failure exits drop their dead `TrustedProxiesCIDR = nil` stores (the function prologue already guarantees the all-or-nothing empty list); and `serverTimingWriter` merges the implicit-WriteHeader prologue into the idempotent `flushHeader()`, deleting the redundant `wrote` flag (it always equaled `injected`). No behavior change — `-race` suites, full lint (both modules), and the negotiator benchmarks stay green; `-t 1` retains only the cross-module `Middleware` alias pair and the standalone doc-snippet script's `if err != nil` pair.
...
- **Release-gate hardening** (`.github/workflows/`, `flake.nix`): the Release workflow re-runs every CI gate (changelog links, doc snippets, module boundaries) on the exact tag commit before publishing — single-publisher design, a tag cut from red code cannot release silently — and its `GO_VERSION` aligns with the 1.27.x matrix. CI gains a lychee link gate (pinned `lycheeverse/lychee-action` v2.9.0), and `lychee` joins the flake devShell so BuildFlow's dev-mode markdown step no longer silently no-ops on a missing binary.

### Removed

- **`etagmetrics` sub-module** — moved to go-etag as the `metrics` package ([`github.com/larsartmann/go-etag/metrics`](https://github.com/larsartmann/go-etag/tree/master/metrics); design record: [go-etag `docs/planning/2026-09-22_23-25_move-etagmetrics-into-go-etag-metrics.md`](https://github.com/larsartmann/go-etag/blob/master/docs/planning/archived/2026-09-22_23-25_move-etagmetrics-into-go-etag-metrics.md)). The adapter counts go-etag's own hooks, so it belongs next to them: it was the only thing forcing this repo to track go-etag v0.4.0 (root pins v0.3.1), every hook-signature change in go-etag required a PR here, and nobody adopting go-etag would look in httputil for its metrics helper. Migration: swap the import — `github.com/larsartmann/httputil/etagmetrics` → `github.com/larsartmann/go-etag/metrics`; `Attach`/`Counters`/`Snapshot` are unchanged (the `HitRatio` formula is corrected there, see Fixed). The v1.3.0 tag keeps the old copy immutable, as always.

### Fixed

- **Nightly fuzz crash #24 was a test-invariant bug, not a `HeaderValue` bug** (`server_timing/server_timing_fuzz_test.go`): the fuzz target counted raw quote parity, so a correctly escaped RFC 7230 quoted-pair (`desc="\""`) read as "unbalanced". The invariant now walks the value as a quoted-string state machine (only unescaped quotes toggle), with regression seeds for the crashing shape; 30s smoke fuzz + `-race` clean.
...
- **LNA spec churn confirmed against live Chrome docs (2026-09-23)**: the `Access-Control-Allow-Private-Network` preflight contract `AllowPrivateNetwork` implements is the Private Network Access draft, which Chrome put on hold — the shipping Local Network Access design (permission prompt, Chrome 142+) gates local-network fetches via a user permission and does not consult this response header. The middleware stays as-is (the header is harmless where unknown and PNA-era Chromium builds honor it); LNA compliance on current Chrome comes from the permission flow, not this header. Recorded in AGENTS.md alongside the field doc.
- `ConfigureNosurfHandler` doc comment now warns that the configuration is applied verbatim with no fallback (unlike `CSRFMiddleware`, a `SameSite=None` + `Secure=false` combination passes through as-is, unstorable in current browsers); `CSRFResponseHeaderMiddleware` doc now pins that it only ever writes a response header and cannot emit cookies, with a test asserting the property.
...
- **Review-pass test hardening**: the two full-code-review passes lost to LLM rate limits were re-run (chain/server/rate-limiter/id-generator tests, all of `scripts/`, the `Example*` scope); three findings fixed — the nonce × SecurityHeaders composition test now pins the documented guidance with an explicit static CSP and asserts the nonce-bearing policy overwrites it (`TestChain_NonceInnerToSecurityHeaders_OverwritesStaticCSP`, replacing a near-duplicate whose config could never exercise a precedence outcome), `TestServerServesRequests` resolves its port via `ListenerAddr` instead of a bind-close-rebind race and drops the stale ":0 never exposes the resolved port" comment, and the pre-commit hook's "staged files" wording now matches its full-package lint run.
- **`CSRFConfig.AllowInsecureSameSiteNone`** (`csrf.go`): opt-out escape hatch that keeps a `SameSite=None` cookie without `Secure` verbatim instead of the constructor's new `Secure=true` fallback. For legacy-client deployments and test rigs only — browsers implementing rfc6265bis §5.7 refuse to store the cookie, so with the flag set every state-changing browser request fails CSRF validation. `Validate` reports the combination either way.
- **Release-gate hardening**: `docs/RELEASE.md` gained an explicit step 12.5 (wait for CI green on the exact tag commit before publishing — the v1.1.0 tag shipped red on the docs-only link check because this step was implicit), and `scripts/prerelease-check.sh` runs `scripts/check-changelog-links.sh` as gate 8b so the local preflight is a superset of CI's changelog-link check.

### Changed

- **Requests without an `Accept-Encoding` header are served uncompressed by default** (`compression.go`, fixes #4): an absent header signals no explicit client preference (RFC 7231 §5.3.4), and minimal HTTP clients, health probes, and intermediaries that do not declare support may not decompress a compressed response — so `Compression` now serves the identity representation to header-less requests instead of compressing with the highest-priority configured encoding. The new `CompressionConfig.AbsentEncoding` field (`AbsentEncodingPolicy`) controls this: the zero value `AbsentEncodingIdentity` is the new safe default (also RFC-correct for an empty header value, which the spec defines as "no content-coding"), and `AbsentEncodingFirstConfigured` restores the exact v1.1.x behavior for deployments whose clients always decompress. `Validate` rejects unknown policy values with the new `compression.absent_encoding_invalid` code (Rejection). Explicit `Accept-Encoding` negotiation (q-values, identity, q=0 exclusions) is unchanged. Migration notes: [docs/migrating-to-v1.2.md](docs/migrating-to-v1.2.md).

- **`http.ErrNoCookie`/`http.ErrNoLocation` are classified Rejection instead of Transient** (`RegisterErrorClassifications`): the named cookie or header location is deterministically absent, so an unchanged retry can never succeed — the package's own taxonomy puts deterministic failures in Rejection. This changes `errorfamily.Classify`/`IsRetryable` results for consumers who route retry decisions on these stdlib sentinels. The `ErrCodeHijackFailed` doc comment and message template were aligned in the same pass (family stays Transient per the documented taxonomy; the template's Fix text no longer implies a deterministic failure). Migration notes: [docs/migrating-to-v1.2.md](docs/migrating-to-v1.2.md).

- **CSRF `TrustedOrigins` parsing is unified and fail-closed** (`csrf.go`): the attestation-consistency check now parses the origin list all-or-nothing, mirroring `nosurf.StaticOrigins` — any entry `url.Parse` rejects leaves no trusted origins at all, so the attestation check can never trust a different set than the nosurf handler enforces (previously a bad entry silently split the semantics: nosurf trusted nothing while the attestation check kept the parseable siblings). `CSRFConfig.Validate` additionally rejects entries that are not usable `scheme://host` origins (unparseable, missing scheme, or missing host) with the new `csrf.trusted_origin_invalid` code (Rejection, cause-chained to `ErrCSRFConfig`) — such entries can never match an Origin header and previously died silently. Construction still follows the validate-and-log contract: an invalid list falls back to same-origin-only validation, loudly logged. Migration notes: [docs/migrating-to-v1.2.md](docs/migrating-to-v1.2.md).

- **CSRF `SameSite=None` without `Secure` falls back to `Secure=true` at construction** (`csrf.go`): a `SameSite=None` cookie without `Secure` is ignored entirely by spec-compliant browsers (rfc6265bis §5.7), so the previously-honored misconfiguration meant the CSRF cookie never persisted and every state-changing browser request failed validation — self-announcing breakage, not silent degradation. The constructor now preserves the config's cross-site intent by forcing `Secure=true` and logs the fallback with the `csrf_samesite_insecure` code; `InvalidateCSRFCookie` applies the same fallback so the deletion cookie matches. `Validate` is unchanged and still rejects the combination; `AllowInsecureSameSiteNone` opts out (legacy-client deployments only). Plain-HTTP deployments behave identically at runtime — the cookie was already unstorable there — but now get a log line naming the fix. This completes the validate-and-log contract's "falls back" step: the combo was the one `Validate()` rejection the constructor honored verbatim (every other rejected config already remediates fail-closed). Design pass + owner ruling: [docs/planning/2026-09-15_csrf-security-degrading-config-design-note.md](docs/planning/2026-09-15_csrf-security-degrading-config-design-note.md). Migration notes: [docs/migrating-to-v1.2.md](docs/migrating-to-v1.2.md).

### Fixed

- **Route patterns are now propagated back up through request-forking middleware** (`requestid.go`, `timeout.go`, `context.go`/`ClientIPMiddleware`, `nonce.go`, `server_timing/server_timing.go`): `net/http.ServeMux` stamps the matched pattern (`r.Pattern`, e.g. `GET /users/{id}`) on the request pointer it receives, so any middleware that re-forks the request with `r.WithContext` before the mux previously left every outer request's `Pattern` empty — silently breaking pattern readers wrapped OUTSIDE the fork (most notably `otelhttp`, whose span names collapsed to the bare method `GET` and whose `http.route` metric attribute vanished whenever it sat above a forking middleware). Each forking middleware now copies the fork's resolved `Pattern` back onto the request it was handed after `next.ServeHTTP` returns. Zero added allocations, cost within benchmark noise (measured pre/post, `-count=10`). Regression-pinned by `TestPatternPropagation*` in both packages; each case fails on its fork site without the fix.

- **`FuzzHealthResponse_Encoding` oracle asserted a false encoder contract** (`health_test.go`): the target required `json.MarshalWrite` to return nil for any status string, but encoding/json v2 legitimately rejects strings with invalid UTF-8 — so the nightly fuzzer's `"\x82"` crasher was a correct encoder refusal misreported as a crash, failing the workflow and auto-filing false-positive issues five nights running (#9–#13, all closed with root-cause comments). The oracle now asserts the contract both ways (invalid UTF-8 ⇒ error; valid UTF-8 ⇒ nil error **and** an exact round-trip, replacing the false "encoder normalizes to U+FFFD" comment), and the minimized seed is committed at `testdata/fuzz/FuzzHealthResponse_Encoding/ac6733e1726c6863`.
...
- **Chain-level exact-fill regression** (`TestCompression_ExactMinSizeWrite_IsNotDuplicated`): the 2026-08-30 duplication bug is now also pinned through the full `Compression()` middleware at the default 512-byte `MinSize`, not just the `compressWriter` unit.
- **`TestChain_RecoveryErrAbortHandler_ThroughStack`**: proves the `http.ErrAbortHandler` re-panic survives the full `Chain` (Nonce + Recovery), not just a bare `Recovery` wrapper.
- **`TestCompression_ZeroLevelMeansDefaultCompression` execution-probe test**: pins that `CompressionConfig.Level == 0` produces default-level gzip (payload round-trips, compressed size well below stored-block size), making the "0 means default" doc claim falsifiable.
- **Middleware-to-middleware composition API** (`compose.go`, `stack.go`): `Compose(mws ...Middleware) Middleware` builds a reusable middleware bundle (first = outermost, matching `Chain`; empty = identity), `MiddlewareStack.Middleware()` exposes a stack as a single nestable middleware with `Build`'s ordering, and `MiddlewareFunc` (a defined type, unlike the `Middleware` alias) carries a `Then(handler)` method for value-level chaining — a nil handler wires a fallback that serves 500 with a self-describing body, so wiring mistakes are diagnosable responses and API functions never panic (the dead `crypto/rand.Read` panic guards in `id_generator.go`/`nonce.go` were deleted too: `rand.Read` is documented never to fail). Ten tests including bundle-nesting and a `Middleware()`/`Build` equivalence check, run 10x under `-race`.
...
- **New fuzz targets**: `FuzzResponseRecorder` (recorder state machine), `FuzzLimitedReadCloser` (bomb-protection boundary), `FuzzDecompressionInvariants` (header removal + round-trip invariants). A nightly CI workflow runs all 23 fuzz targets for 5 minutes each (the 2026-08-30 pass added the 15 that were missing, including `FuzzCompression` — the target that caught the exact-fill duplication bug).
...
- **httpspec CORS origin-matching spec** (`SpecNameCORSOriginMatchesRequested`): `CORSSpecs()` grows to 5 specs (26 total with rate-limit specs). Verifies that when a response authorizes a specific (non-wildcard) origin, that origin is the one the client requested — catches handlers that stamp a hardcoded, mismatched origin onto every response. Static `*` and denials pass.
- **Compression+ETag Hijack passthrough coverage restored** (`chain_hijack_test.go`): the chain-level interface test was lost in the go-etag extraction (`cc6439e`) and the real-connection upgrade test in the websocket-test removal (`485cc82`). Restored as three tests: interface preservation through Compression+ETag, a light real-server upgrade-response test (101 + no injected `Content-Encoding`/`ETag`), and a full bidirectional byte-integrity test over the hijacked connection (stable across `-race -count=10`).
- **Negotiation property tests** (`compression_negotiator_property_test.go`): 2,000 seeded pseudo-random Accept-Encoding headers checked against an independent reference model (highest q wins, ties by server preference, q=0 never selected, unavailable encodings never selected) plus header-order invariance under shuffling.
- **`responseWrapper` direct tests** (`wrapper_test.go`): first-WriteHeader-wins, buffered-status commit semantics (`writeHeaderToUnderlying` commits exactly once), implicit-200 behavior, and Hijack/Flush delegation — previously covered only indirectly through the compress/etag writers.
- **`BenchmarkETagAdapterOverhead`**: proves the deprecated `httputil.ETag` adapter is a zero-cost passthrough over `etag.New` (identical allocs; latency within noise) with a no-middleware baseline for absolute context.
- **`BenchmarkHTTPRequestConstruction`** + [profiling note](docs/research/2026-08-30_httptest-newrequest-profiling.md): `httptest.NewRequest` costs ~760 ns / 5104 B / 9 allocs vs ~110 ns / 512 B / 3 allocs for `http.NewRequestWithContext` — the falsifiable evidence behind the noctx test-file suppression.
...
- **`CompressionConfig.Level = 0` has one documented meaning**: "unset → `gzip.DefaultCompression`". The constructor's remap stays, `Validate` documents why `0` passes, `DefaultWriterFactoriesForLevel` documents that its raw level parameter follows stdlib gzip semantics (`0` = `NoCompression` there), and the Level range check applies only when `WriterFactories` is empty — the only case where Level drives factory construction — so configs that set both no longer produce a spurious warning (the constructor checks the level at its use site instead).
- **`DefaultCSRFHeaderName` value canonicalized from `"X-CSRF-Token"` to `"X-Csrf-Token"`** (`csrf.go`): Go's canonical MIME header form, as enforced by the `canonicalheader` linter (which tracks constants through every `Header.Set`/`Get` use site). Functionally identical on the wire — HTTP header names are case-insensitive and all reads/writes go through canonicalizing methods; verified against nosurf v1.2.0, which compares via `Header.Get`. Consumers doing exact-string matching against the old spelling should switch to the constant or the canonical value.
- **`compressWriter.Close` is idempotent** (`compress_writer.go`): a second `Close` used to dereference the nilled writer (nil-interface panic) because `compressing` stayed true; both success and error paths now reset the state. Regression tests cover both paths.
- **Unknown custom encodings sort truly alphabetically** (`compression_negotiator.go`): the tie-break key for encodings outside the built-in preference list was a base-256 byte accumulation that overflowed `int` for names of nine or more bytes, making the documented deterministic order undefined for long names; unknown encodings now get a lexicographic rank (stable, overflow-free). Affects only tie-break order among custom encodings.
- **`KeyedRateLimiterConfig.MaxKeys` doc states the refresh semantics** (`ratelimit_keyed.go`): the access stamp is refreshed on the slow path only, so eviction victim choice is approximate within one TTL window (behavior unchanged; contract now documented).
- **Decompression benchmarks modernized to `b.Run` groups** (`BenchmarkDecompression/gzip|deflate|passthrough`, replacing three top-level benchmarks — 08-52:f49).
- **`RunSerial` state-sharing decided**: specs stay self-contained by design; the reasoning is recorded in [docs/planning/archived/2026-08-30_runserial-state-sharing-design-note.md](docs/planning/archived/2026-08-30_runserial-state-sharing-design-note.md) (07-45:f47).
- **Test naming/assertion convention codified** in AGENTS.md (name = claim, `Test<Component>_<Behavior>[_Condition]`, mutation-checked preservation tests) — the rule born from the misleading `HijackPassthrough` test name (06-12:f.6).
- **Integration-docs audit result** (`docs/integrations/redis-ratelimiter.md`): the Redis example teaches the deprecated `RateLimiter`/`RateLimit()` API; the doc now carries an explicit deprecation notice with the migration-guide pointer and states that `KeyedRateLimiter` has no pluggable backend yet. All other integration docs' import paths verified current (`06-44:f16`).
...
- **`TestNonce_UniquePerRequest` only compared every nonce against the first** (`nonce_test.go`, found by the T13 line-by-line review): a nonce repeating at index 3 and 5 (both differing from index 0) would have passed. The check is now pairwise via a set. Same review fixed `TestSecurityHeaders_CustomHeadersEmptyMap`, which asserted against stale header names (`X-Custom` instead of the configured `X-Custom-Header`).
- **httpspec `varyContainsToken` renamed to `varyContainsOrigin`** (`httpspec/cors_ratelimit_specs.go`): the `token` parameter received `"Origin"` at every call site (an `unparam` finding present at the 2026-09 lint-config rework), so the helper now encodes its real contract in its name and signature.
- **`Logging` middleware dropped the request ID** (`logging.go`, fixes #2): the middleware logged method/path/status/duration/client_ip but not the ID that `RequestID` stores in the request context, so log lines could only be correlated with `X-Request-Id` response headers by hand. The log now carries `request_id` (omitted entirely, never empty, when no ID is in context) and is emitted through `context.WithoutCancel` so canceled request contexts — `Timeout` deadlines, client disconnects — can never make cancellation-aware slog handlers drop the very logs you need. Regression: `TestLogging_IncludesRequestIDMatchingResponseHeader`, `TestLogging_OmitsRequestIDWhenAbsent`, `TestLogging_EmitsWhenRequestContextCanceled`.
- **httpspec `SpecNameNoDuplicateHeaders` false-positived on legal multi-line headers** (`httpspec/specs.go`, fixes #3): the check failed on ANY header with multiple values, but `Vary`-style list-syntax fields are combinable into one comma-joined line (RFC 9110 §5.2), multiple `Www-Authenticate`/`Proxy-Authenticate` challenges are explicitly blessed as separate lines (§11.6.1), and `Set-Cookie` can never be combined (§5.6.1) — the exact `Vary: Cookie` + `Vary: Accept-Encoding` shape CSRF-cookie plus compression middleware produce together forced consumers to skip the whole spec. Exactly those six list-valued headers are now exempt; single-valued duplicates and mixed-casing writes still fail. Regression: `TestNoDuplicateHeadersPassesForListValuedHeaders`.
- **CI never set `GOEXPERIMENT=jsonv2`** (`.github/workflows/ci.yml`): the root package has imported `encoding/json/v2` since the 2026-08-16 migration, so every CI run since failed at `go build` with "build constraints exclude all Go files" — masking all downstream gates. The experiment flag is now set for both jobs.
- **Compression duplicated response bytes when a Write exactly filled the buffer** (`compress_writer.go` `writeBuffered`, critical, found by `FuzzCompression` 2026-08-30): when a single `Write` brought the buffer to exactly `minSize` bytes (or a chunk boundary did), the payload stayed in the write argument after being copied into the buffer, so the flush/stream step wrote the same bytes twice — every response whose chunking exactly hit the threshold was corrupted (body present twice once gunzipped). The consumed branch now clears the buffer argument. Regression: `TestCompressWriter_ExactMinSizeWrite_IsNotDuplicated` + fuzz seed.
- **`Recovery` swallowed `http.ErrAbortHandler`** (`recovery.go`): the stdlib sentinel for silent mid-stream aborts was caught, logged as a panic with a full stack trace, and a doomed 500 was written to the aborted connection. The middleware now re-panics the sentinel unchanged so `net/http`'s own silent handling applies. Regression: `TestRecovery_RepanicsErrAbortHandler`.
- **Two error-message templates claimed `0` disables a limit when it does not** (`errors.go`): `maxbodysize.max_bytes_negative`'s fix text said "zero (unlimited)" — a zero limit actually rejects any non-empty body (probed against `net/http`); `decompression.max_size_negative`'s fix text said "zero (no limit)" — zero selects the 16 MiB default. Both templates now state the real semantics, matching the README/source corrections from the 2026-08-30 bomb-protection study.
- **`Decompression` benchmark harness measured the wrong path** (`decompression_bench_test.go`, `docs/benchmarks.md`): the middleware deletes `Content-Encoding` from the request it serves, so benchmarks that reused one request decompressed exactly once and then measured millions of passthrough no-ops (the old table's impossible "160 allocs at 137 ns" rows). The harness now restores the header per iteration; real numbers: gzip 16 KiB ≈ 7.3 µs/op (2.3 GB/s), passthrough ≈ 106 ns/op.
- **README/source claimed `MaxDecompressionSize: 0` disables bomb protection** — the constructor maps `0` to the 16 MiB default (verified by execution: a 20 MiB body trips the limit). README (2 sites) and the field comment now document the real semantics: `0` selects the default; there is no "unlimited" option.
- **Two undocumented error discards** now carry their "honest silence" justification: the `validateConfig` slog `Handle` drop (`recorder.go`) and the compressing-branch `Flush` drop (`compress_writer.go`) — both found in the 06-50:f39 sweep; every other discard site was already documented.
...
- **`MiddlewareDecompression` constant** (`stack.go`): Decompression was the only middleware missing a `Middleware*` stack name constant. All built-in middlewares now have named constants for `MiddlewareStack.Add()`.
