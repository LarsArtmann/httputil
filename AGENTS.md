...
- **`ETag` `If-None-Match` uses the RFC 7232 §2.3.2 weak comparison function** — `W/"abc"` and `"abc"` are treated as equivalent (the `W/` weakness indicator is ignored). `parseETagList` is quote-state-aware so commas inside quoted opaque-tags do not split the list. The `parseETagList` allocation (1 alloc / 16-64 B per conditional request) was benchmarked and is acceptable (<5% of the full middleware path); see `BenchmarkETagInList`.
