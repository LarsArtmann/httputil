...
**Middleware pattern:** All middleware is `func(http.Handler) http.Handler` (aliased as the `Middleware` type in `recorder.go`). `Chain()` applies them in declaration order (first = outermost) via `slices.Backward`; `Compose()` bundles middleware into one reusable `Middleware` with the same ordering. `MiddlewareStack` collects named middleware with duplicate prevention and ordering validation (Recovery must be outermost when present); `MiddlewareStack.Middleware()` exposes the stack as a single nestable middleware (reads current entries per application, same ordering as `Build`).
...
- **`RequestID` default generator** produces a 16-byte time-ordered ID (Unix seconds + atomic counter + random tail) and amortizes `crypto/rand` syscalls across ~256 IDs via a process-wide buffer. Known tradeoff: slow readers copy from the shared buffer without holding `refillMu`, so a copy can interleave with a refill write — a formal data race whose worst case is a torn read of two independent crypto/rand fills (output still random). Fixing it properly needs a generation-stamped ring (see TODO_LIST); don't "fix" it with a plain pointer swap, which reintroduces duplication across generations.
