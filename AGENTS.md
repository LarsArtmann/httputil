...
**Parallel-writer protocol (2026-09-23 restore-war lesson):** this fleet also runs parallel agent sessions, and "vanished files" can be another writer's deliberate removal, not lost work. Before restoring anything, run `git log --format='%h %s' -10` and scan for NON-heuristic commit messages touching that path (a reasoned message like `chore(dependabot): drop the /etagmetrics module entry` is a deliberate act; `chore: auto-commit N file(s)` is the daemon). Deletions get an immediate deliberate commit — the daemon's snapshot logic resurrected a deleted directory three times and even committed it back once (`965b213` vs `362c4ce`, 2026-09-22/23). The same fight produced the broken `go 1.27` directive on post-v1.3.0 master (fixed 2026-09-23): treat version-directive downgrades that contradict a dependency's `go` requirement as daemon casualties, verify against the tag, and restore. Never cite daemon heuristic commits in external artifacts (issue comments, docs) — reference CHANGELOG sections, tags, or deliberate commits.
...
- **Ghost-module enumeration (2026-09-23):** after deleting a module directory, buildflow's golangci step can still enumerate and LINT the deleted directory (stale workspace snapshot); `BUILDFLOW_NO_RESULT_CACHE=1` does not help. The fix is on-disk reality: confirm the directory is really gone (`ls`), then re-run — if the lint failure names a nonexistent path, the tree, not the cache, is the problem.
- **Sandboxed treefmt vs toolchain download (2026-09-23):** after the go-directive repair, `nix flake check` failed its treefmt gate: nixpkgs goimports shells out to `go` (go/packages import resolution), the sandbox has no network, and GOTOOLCHAIN=auto tried to download the go 1.27.1 the directive names. Fixed in `flake.nix` by wrapping goimports with `GOTOOLCHAIN=local` + `go_1_27` (1.27.1 satisfies the directive without any download) and a TMPDIR GOCACHE fallback. Symptom signature: `nix log` of the treefmt-check drv shows `go: download go1.27.1: ... connection refused` — same pattern applies to ANY go toolchain call in a sandboxed check after a directive bump.
- **Snapshot before `--fix`:** run `git status --short > /tmp/pre-buildflow.txt` before any `buildflow … --fix` step — with a foreign writer active in the same window, attribution of working-tree changes becomes guesswork without the pre-snapshot (2026-09-23 misattribution incident).
...
- **`CORS` `AllowPrivateNetwork` is preflight-only and opt-in** — when true, only the middleware-generated preflight 204 carries `Access-Control-Allow-Private-Network: true` (Chrome's Private Network Access / Local Network Access check). Never set on actual requests and never with `OptionsPassthrough` (the handler owns the preflight there). It is sent unconditionally on the preflight rather than echoed from the request header, matching the Chromium-verified consumer implementation (dnsblockd `server/cors.go`) — do not \"simplify\" it into an echo, a Chrome-side signal change would silently break every LNA-gated fetch. Default `false`. **Spec-churn note (live Chrome docs, 2026-09-23):** the preflight-header contract is the PNA draft, which Chrome put on hold; the shipping Local Network Access design (permission prompt, Chrome 142+) gates local-network fetches via a user permission and does not consult this response header. The middleware stays as-is — the header is harmless where unknown, documents what it does, and PNA-era Chromium builds still honor it — but LNA compliance on current Chrome comes from the permission flow, not this header.
