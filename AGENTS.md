...
| `ratelimit.go`                | `RateLimit()`, `RateLimiter`, `TokenBucketLimiter`, `NewTokenBucketLimiter()`, `RateLimitConfig`, `DefaultRateLimitConfig()`, `Validate()`                                  | Token bucket rate limiting with pluggable limiter interface        |
...
- **`TokenBucketLimiter` has unbounded memory** — the per-key `buckets` map grows without eviction. For per-IP rate limiting behind a large proxy, provide a custom `RateLimiter` implementation with eviction (e.g., Redis-backed).
