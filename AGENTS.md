...
| `errors.go`                   | `ErrCodeWriteFailed`, `ErrCodeHijackUnsupported`, `ErrCodeHijackFailed`, `ErrCodeCompressWriteFailed`, `ErrCodeETagWriteFailed`, `ErrCodeETagConfigInvalid`, `ErrCodeETagHashWriteFailed`, `ErrETagConfig`, `RegisterErrorClassifications()`                              | Error codes + stdlib sentinel registration + message templates                               |
...
| `wrapper.go` | (unexported `responseWrapper`) | Shared ResponseWriter wrapper for compress writer and ETag writer |
...
| `hex.go` | (unexported `hexDigitsLower`, `hexEncodeUint64`) | Shared lowercase hex lookup table + uint64 encoder for RequestID and ETag |
...
| `entity_tag.go` | `EntityTag`, `EntityTagStrength`, `EntityTagStrong`, `EntityTagWeak`, `NewEntityTag()`, `ParseEntityTag()`, `ParseEntityTagList()`, `MatchesIfNoneMatch()`, `MatchesIfMatch()` | RFC 7232 §2.3 entity-tag domain type: opaque value + strength, strong/weak comparison, ABNF parser |
| `etag.go` | `ETagConfig`, `DefaultETagConfig()`, `ETag()`, `Validate()` | ETag middleware: FNV-64a generation, If-None-Match 304, buffer overflow, Hijack/Flush streaming |
...
| `ETag`     | `http.etag_write_failed`     | Transient      | Yes       | ETag writer streaming, overflow, or flush write failure       |
| `ETag`     | `http.etag_config_invalid`   | Rejection      | No        | ETagConfig field value is invalid (e.g. MaxBufferSize <= 0)   |
| `ETag`     | `http.etag_hash_write_failed`| Orchestration  | No        | Hash.Write returned an error (contract violation)             |
...
- **Post-header-commit writes discard errors by design ("honest silence")** — after `WriteHeader` is called, the HTTP response is in-flight and write failures cannot be surfaced to the client or caller. All such sites use `_, _ = w.Write(...)` or `_ = closer.Close()` with an inline comment explaining why. Sites: `compressWriter.Flush()` (post-handler body flush); `Recovery()`, `CSRFMiddleware()`, `KeyedRateLimiterMiddleware()`, `RateLimit()` (error-response writes after committed status); `Compression()` defer Close (cleanup); `limitedReader.Read()` bomb-protection Close; `etagWriter.flush()` and `etagWriter.Flush()` (post-handler body flush).
- **`ETag` middleware buffers GET and HEAD responses** — it computes an FNV-64a hash of the body, sets the `ETag` header, and handles `If-None-Match` with 304 Not Modified. The domain type is `EntityTag` (not `ETag`) to free up `ETag()` as the middleware constructor name (matching `CORS()`, `Compression()`, etc.).
- **`ETag` buffer overflow disables ETag** — responses exceeding `MaxBufferSize` (default 1 MB) are streamed without an ETag. Hijack and Flush calls switch to streaming mode immediately.
- **`ETag` `SkipIfPresent` defaults to false** — set to true to respect handler-set ETags. When the handler's ETag is absent or malformed, the middleware falls back to computing one.
- **`ETag` `OnError` callback** — post-commit write failures (streaming, overflow, flush) are forwarded to `OnError func(*errorfamily.Error)` for observability, since they cannot be surfaced to the client. When nil, errors are silently dropped.
- **`ETagConfig` zero-value is safe** — `MaxBufferSize <= 0` is clamped to 1 MB in `newETagWriter`. A bare `ETagConfig{}` won't cause unbounded buffering.
- **`EntityTag` `If-None-Match` uses RFC 7232 §2.3.2 weak comparison** — `W/"abc"` and `"abc"` are treated as equivalent.
- **`MatchesIfMatch` is an exported helper** — applications call it in their handlers for unsafe-method lost-update prevention (RFC 7232 §3.1 strong comparison).
- **`hijackDelegate` attaches `writer_type` context** — both error branches (`ErrCodeHijackUnsupported`, `ErrCodeHijackFailed`) include `WithContextf("writer_type", "%T", w)` for debugging which ResponseWriter type failed.
