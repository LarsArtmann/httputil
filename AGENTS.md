...
Two packages: the flat `httputil` package (middleware + server lifecycle) and the `httputil/httpspec` subpackage (reusable HTTP behavior specs). Three external dependencies: `github.com/larsartmann/go-error-family`, `golang.org/x/time`, and `github.com/justinas/nosurf`. Go 1.26+.

### Why the Root Package Is Flat (Deliberate, Not Debt)

The root `httputil` package has 33 non-test files in one directory. This is the Go-idiomatic choice for a utility library — consumers want `httputil.CORS()`, not `httputil/cors.CORS()`. Public sub-packages are structurally impossible: compression depends on root symbols (`Middleware`, `responseWrapper`, `ErrCode*`), creating circular imports if extracted. An `internal/` extraction is technically viable (root → internal is one direction, no cycles) but deferred until post-v1.0 or if root exceeds ~50 non-test files. See `docs/architecture-understanding/2026-08-05_06-56_package-structure-analysis.html` and `docs/modularization/2026-08-05_DECISION.html` for the full analysis.
...
| `ratelimit_keyed.go`          | `KeyedRateLimiter`, `NewKeyedRateLimiter()`, `KeyedRateLimiterMiddleware()`, `KeyedRateLimiterConfig`, `DefaultKeyedRateLimiterConfig()`, `KeyExtractor`, `KeyExtractorFromRemoteAddr()`, `KeyExtractorFromClientIP()`                                                                                                                                                                                                                   | Keyed rate limiting with O(log n) min-heap eviction + `MaxKeys` cap                          |
...
| `server_timing.go`            | `ServerTiming`, `NewServerTiming()`, `ServerTimingMiddleware()`, `ServerTimingMiddlewareWhen()`, `MeasureServerTiming()`, `WrapServerTiming()`, `RecordServerTiming()`, `WithServerTiming()`, `ServerTimingFromContext()`                                                                                                                                                                                                                | W3C Server-Timing header instrumentation with CRLF-safe values                               |
...
- **`ServerTimingMiddleware` injects `*ServerTiming` via context** — use `ServerTimingFromContext(ctx)` or `MeasureServerTiming(ctx, name)` to record metrics inside handlers. `WrapServerTiming(w, r)` provides manual wrapping without the middleware. Header values are sanitized against CRLF injection.
...
- **Test files split by middleware** — each middleware has its own `*_test.go` (e.g., `security_test.go`, `requestid_test.go`, `recovery_test.go`, `timeout_test.go`, `logging_test.go`, `csrf_test.go`, `server_timing_test.go`, `ratelimit_keyed_test.go`). Compression middleware tests are in `compression_test.go` and `compression_negotiator_test.go`; q-value parsing tests are in `compression_qvalue_test.go`; factory tests are in `compression_factory_test.go`; the ID generator has `id_generator_test.go`. Chain integration tests in `chain_test.go`. Server-Timing also has `server_timing_bench_test.go` and `server_timing_fuzz_test.go`.
