...
- **Workspace-vendor format (2026-09-11 incident):** go1.26.7 writes `## explicit; go <ver>` markers (both `go mod vendor` and `go work vendor`) and a `## workspace` header (work vendor only). BuildFlow's gomod-check parser matched only bare `## explicit`, producing mass "not marked as explicit" false positives; fixed upstream in `parseVendorModulesTxt` with regression tests. If those false positives reappear, the installed binary is stale.
- **Forced `-mod=vendor` quirk:** `go work vendor` output omits the trailing versionless replace-marking header, so `GOWORK=off go build -mod=vendor ./...` fails with "server_timing: ... not marked as replaced". Latent and harmless — plain and workspace builds pass, and no gate forces `-mod=vendor`. The `go mod vendor` format works in both modes but is churn-reverted by the next `go work vendor`; the workspace format stays canonical.
- **Accepted info finding:** gomod-check/go-mod-ignore-check report "directory vendor exists but is not ignored in go.mod". An `ignore ./vendor` directive was A/B-tested as functionally neutral but deliberately NOT added — the real top-level `vendor/` is special-cased by the go toolchain, and the convention in this ecosystem is to ignore only non-Go directories (e.g. `ignore ./node_modules`).
